...
>>>>>>> (#1860) - Fix info.doc_count for leveldb
            if (err) {
              results.push(err);
            } else {
              results.push(doc);
  ...
  function removeSeq(seq, callback) {
    stores.bySeqStore.del(formatSeq(seq), callback);

  }

  function removeRev(rev_tree, rev, depth) {
    ...
    if (rev_tree[2].length === 0) {
      ...
      if (removeRev(rev_tree[2][0], rev, depth + 1)) {
        return true;
      }
      return false;
    } else {
      for (var i = 0; i < rev_tree[2].length; i++) {
        if (removeRev(rev_tree[2][i], rev, depth + 1)) {
  ...
  function purgeOneRev(docId, docRev, callback) {
    stores.docStore.get(docId, function (err, res) {
      ...
      if (removeRev(revTree, docRev, 0)) {
        var revMap = myCopy.rev_map;
        removeSeqList(revMapToSeqList(revMap), function (err2, res2) {
          if (err2) {
            callback(err2);
            return;
          }
