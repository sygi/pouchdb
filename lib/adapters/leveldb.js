...
var updateSeqKey = '_local_last_update_seq';
var docCountKey = '_local_doc_count';
...
function LevelPouch(opts, callback) {
  ...
  function withDB() {
    ...
    stores[bySeqStore].get(updateSeqKey, function (err, value) {
      if (!err) {
        update_seq = value;
      }
      else {
      ...
      stores[bySeqStore].get(docCountKey, function (err, value) {
        if (!err) {
          doc_count = value;
        }
        else {
  ...
  api._info = function (callback) {
    ...
    stores[bySeqStore].get(docCountKey, function (err, _doc_count) {
      if (err) { _doc_count = doc_count; }

      stores[bySeqStore].get(updateSeqKey, function (err, _update_seq) {
  ...
  api._bulkDocs = function (req, opts, callback) {
    ...
    function insertDoc(doc, callback) {
      ...
      writeDoc(doc, function () {
        stores[bySeqStore].put(docCountKey, doc_count, function (err) {
          if (err) {
            // TODO: handle error
          }
          return callback();
    ...
    function saveUpdateSeq(callback) {
      stores[bySeqStore].put(updateSeqKey, update_seq, function (err) {
        if (err) {
          // TODO: handle error
        }
        return callback();
  ...
  function removeSeq(seq, callback){
    stores[BY_SEQ_STORE].del(formatSeq(seq), callback);
  }
  ...
  api.purge = function(docId, callback) {
    stores[DOC_STORE].get(docId, function(err, doc){
      removeSeqList(revMapToSeqList(doc.rev_map), function(){
        stores[DOC_STORE].del(docId, function(err){
          callback();
        });
      });
    });
