 ...
 * Data in levelDB adapter is kept in the four stores:
 * - document store keeps revision data, such as
 *   revision history or revision-sequence map
 * - by-sequence store keeps every revision attributes (data)
 *   and a mapping of consecutive integers to that data
 * - attachment store keeps a mapping from attachment numbers
 *   to the documents ids to which attachment belongs
 * - binary store keeps actual binary data concerning the attachments
 *   which are integer mapped
 *
 * Elements of a doc-store entry:
 * - id
 * - current revision (rev)
 * - revisions:
 *   - start =?
 *   - ids - current revisions for every depth in the rev tree
 * - rev_tree: 
 *   - list =?
 *   - pos =?
 *   - ids:
 *     - first field denotes revision of a node in a tree
 *     - second if it is downloaded and stored in seq store
 *     - third contains list of the children of that node 
 *       (in the same format)
 * - rev_map - mapping from revision numbers (with additional tree depth)
 *     to the integers denoting positions in by-sequence store
 * - seq - current revision number in seq store (=rev_map[rev])
 *
 *  Example:
 * { id: 'costam',
 *   rev: '2-9f9977b36f977adea007c9d9c86d6e8a',
 *   revisions: 
 *   { start: 2,
 *     ids: 
 *       [ '9f9977b36f977adea007c9d9c86d6e8a',
 *         'f4a51e7fb96c58c9c565762a450a0301' ] },
 *   rev_tree: 
 *     [ { pos: 1,
 *         ids:
 *           [ 'f4a51e7fb96c58c9c565762a450a0301',
 *           { status: 'available' },
 *             [ 
 *             [ '232c631040fa59a37ff9a5b98cf51aac', 
 *               { status: 'available' }, 
 *               [] ],
 *             [ '9f9977b36f977adea007c9d9c86d6e8a',
 *               { status: 'available' },
 ...
 *     } ],
 *   rev_map: 
 *    { '1-f4a51e7fb96c58c9c565762a450a0301': 1,
 *      '2-232c631040fa59a37ff9a5b98cf51aac': 2,
 *      '2-9f9977b36f977adea007c9d9c86d6e8a': 3 },
 *   seq: 3 }
 */
    ...
    stores.docStore.get(docId, function (err, res) {
      ...
      removeRev(revTree, docRev, 1, revMap, function (value, purged) {
        if (value) {
          ...
          myCopy.rev = merge.winningRev(myCopy);
          myCopy.seq = myCopy.rev_map[myCopy.rev];
          myCopy.revisions.ids[0] = removeSeqNumber(myCopy.rev);
          //what is metadata.revisions.start?
