...
function LevelPouch(opts, callback) {
  ...
  function removeRev(rev_tree, rev, depth, rev_map) {
    ...
    if (rev_tree[2].length === 0) {
      ...
      if (removeRev(rev_tree[2][0], rev, depth + 1, rev_map)) {
        removeFromRevMapAndSeqStore(depth + "-" + rev, rev_map, function () {
          return true;
        });
        return true;
      ...
      for (var i = 0; i < rev_tree[2].length; i++) {
        if (removeRev(rev_tree[2][i], rev, depth + 1, rev_map)) {
...
/*  function revMapToSeqList(rev_map) {
  ...
  function purgeOneRev(docId, docRev, callback) {
    stores.docStore.get(docId, function (err, res) {
      ...
      if (removeRev(revTree, docRev, 1, revMap)) {
